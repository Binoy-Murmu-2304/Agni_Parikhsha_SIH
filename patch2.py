...
content = content.replace('if len(df_def) < n_def: df_def = df_def.sample(n=n_def, replace=True)', 'if len(df_def) < n_def: raise ValueError(f"Insufficient def")')
content = content.replace('if len(df_neg) < n_neg: df_neg = df_neg.sample(n=n_neg, replace=True)', 'if len(df_neg) < n_neg: raise ValueError(f"Insufficient neg")')
