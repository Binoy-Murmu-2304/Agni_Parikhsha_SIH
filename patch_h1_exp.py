...
# Remove alias mechanism_map
old_explain = '''        # Alias collinear base features to their dominant physical mechanisms for the cards
        mechanism_map = {}
...
new_explain = '''        contributions = []
        for name, val in zip(feature_names, vals):
            contributions.append({"feature": name, "shap_value": val})'''

c = c.replace(old_explain, new_explain)

# Rewrite get_physics_narrative
old_narrative = '''    def get_physics_narrative(self, feature_name: str, shap_val: float) -> str:
        """
...
new_narrative = '''    def get_physics_narrative(self, feature_name: str, shap_val: float) -> str:
        """
...
c = c.replace(old_narrative, new_narrative)
